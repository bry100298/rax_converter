...
# Set JAVA_HOME to custom JRE path
# os.environ['JAVA_HOME'] = os.path.join(os.getcwd(), 'custom_jre')
...
def pdf_to_html(pdf_file, output_folder):
    start_jvm()  # Start the JVM before using tabula
...
def html_to_excel(html_file, parent_dir, pdf_file):
    ...
    stop_jvm()  # Shutdown the JVM after completing the PDF to Excel conversion
